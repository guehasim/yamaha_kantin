  ...
  */
  class Datatables
  {
    ...
    */
    ...
    protected $sub_querys     = array();
    protected $where          = array();
    protected $filter         = array();
    protected $add_columns    = array();
    protected $edit_columns   = array();
    protected $unset_columns  = array();
    protected $modul          = array();
    ...
    */
    ...
    /**
    * custom function to generate sub data query
    * @param string $column 
    * @param string $data_query 
    * @param string $match_replacement
    */

    public function sub_query($column, $data_query, $match_replacement = NULL, $type = 'array') {
      $this->sub_querys[$column] = array(
    ...
    */
    ...
    public function order_by($columns = '', $type = '') {
      $this->ci->db->order_by($columns, $type);
    }
    ...
    */
    protected function get_ordering()
    {
      ...
      for($i = 0; $i < intval($this->ci->input->post('iSortingCols')); $i++) {
        if(isset($mColArray[intval($this->ci->input->post('iSortCol_' . $i))]) && $this->ci->input->post('bSortable_'.intval($this->ci->input->post('iSortCol_' . $i))) == 'true') {
    ...
    */
    protected function get_filtering()
    {
      ...
      $mColArray = array_values(array_diff($this->columns, $this->unset_columns));
      $columns = array_values(array_diff($this->columns, $this->unset_columns));

      // print_r($mColArray);exit;

      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns))
            if(!empty($this->select[$mColArray[$i]])){
                if(strpos($this->select[$mColArray[$i]], "SUM(") === false){
                  if(strpos($this->select[$mColArray[$i]], "DISTINCT") !== false){
                    if(strpos($this->select[$mColArray[$i]], "SELECT") !== false){
    ...
    */
    protected function produce_output($charset)
    {
      ...
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        ...
        foreach ($this->sub_querys as $field => $val) 
          $aaData[$row_key][$field] = $this->exec_query($val, $aaData[$row_key]);
    ...
    */
    ...
    protected function exec_query($custom_val='', $row_data)
    {
      $replace_string = '';
      ...
      if(isset($custom_val['replacement']) && is_array($custom_val['replacement']))
      {
        foreach($custom_val['replacement'] as $key => $val)
        {
          ...
          $custom_val['data_query'] = str_ireplace('$' . ($key + 1), $replace_string, $custom_val['data_query']);
        }
      }

      if (!empty($custom_val['data_query'])) {
        $data = $this->ci->db->query($custom_val['data_query']);
        if ($data->num_rows() > 0) {
          if ($custom_val['type'] == 'array') {
