...
class M_minum extends CI_Model
{
    public function get()
    {
        ...
        $this->datatables->add_column(
            'action',
            '<a data-id="$1" href="javascript:void(0)" class="btn btn-detail-minum btn-xs btn-circle btn-primary" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-pencil"></i></a> 
            <a href="'.site_url('minum/delete').'/$1" class="remove-record btn btn-xs btn-circle btn-danger" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></a>',
            'ID_menu'
        );
    ...
    {
        $query = $this->db->query("SELECT * FROM ym_menu WHERE jenis = '1' ");
        return $query->result();
    }
    ...
    public function delete($id)
    {
        if (!empty($id)) {
            try {
                ...
                return json_encode(array('error' => 'Data Minuman tidak dapat dihapus karena berelasi dengan data lain'));
            }
        }
